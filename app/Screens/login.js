...
//               <Text className="text-2xl font-bold text-gray-800">Login to Your Account</Text>
//             </View>
...
//                 <Text className="text-green-500 font-semibold text-sm">Sign Up</Text>
//               </TouchableOpacity>
//             </View>
//           </Animated.View>
//         </ScrollView>
...
const LoginPage = ({ navigation }) => {
  ...
  const checkSavedCredentials = async () => {
    try {
      ...
      if (savedEmail && savedPassword) {
        // You might want to auto-login or pre-fill the form
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (savedEmail && savedPassword) {
          ...
          Alert.alert('No Saved Credentials', 'Please login manually first to save credentials');
        }
      }
    } catch (error) {
      Alert.alert('Authentication Failed', error.message || 'Biometric login failed');
 ...
 const handleLogin = async (data) => {
  ...
  try {
    ...
    navigation.navigate(navigationMap[decodedToken.role] || 'Home');
  ...
  try {
    ...
    navigation.navigate(navigationMap[response.data.user.role] || 'Home');
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      // You'll need to use @invertase/react-native-apple-authentication
      // Then call your backend API similar to handleSocialLoginSuccess
    } catch (error) {
      Alert.alert('Apple Login Failed', error.message || 'Something went wrong');
    } finally {
      setSocialLoading(null);
